...
app.use(session({
  secret: process.env.SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || 'fallback-secret-key',
  resave: false,
...
// Registrar partials manualmente
hbs.getPartials().then(partials => {
  console.log('Partials registrados:', Object.keys(partials));
}).catch(err => {
  console.log('Error registrando partials:', err);
});
